package cn.com.navia.MysqlTodayData;
...
public class update {
	...
	public static void update_analyse_active(update db,int active_count,int cur_active_count,int new_count,int lost_count,int re_active_count,int id) throws SQLException{
		String sql="update analyse_active set active_count="+active_count+",cur_active_count="+cur_active_count+",new_count="+new_count+",lost_count="+lost_count+",re_active_count="+re_active_count+" where id='"+id+"'";
		int ret=db.stmt.executeUpdate(sql);
		if(ret!=1){
			System.out.println("insert_analyse_stay_ext err!");
		}
	...
	public static void update_data() throws SQLException{
		...
		while(rs.next()){
			...
			int avg_stay_time=(int) (rs.getInt("avg_stay_time")*getPersent());
